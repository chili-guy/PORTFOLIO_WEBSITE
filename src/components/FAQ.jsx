...
const NodeItem = React.memo(({ question, answer, index, isOpen, onToggle, side, totalItems }) => {
    ...
    let orbitOffset = '';
    if (side === 'left') {
        if (isTop) orbitOffset = 'lg:translate-x-8 lg:-translate-y-4';
        else if (isBottom) orbitOffset = 'lg:translate-x-8 lg:translate-y-4';
        else orbitOffset = 'lg:-translate-x-8';
    } else {
        if (isTop) orbitOffset = 'lg:-translate-x-8 lg:-translate-y-4';
        else if (isBottom) orbitOffset = 'lg:-translate-x-8 lg:translate-y-4';
        else orbitOffset = 'lg:translate-x-8';
...
const FAQ = () => {
    ...
    return (
        <section className="relative min-h-[900px] lg:min-h-[1100px] py-20 flex flex-col justify-center overflow-hidden bg-black" id="faq">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 overflow-hidden mix-blend-screen translate-y-[10%]">
