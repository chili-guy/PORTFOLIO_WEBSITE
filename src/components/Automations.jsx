...
import { Lock, Play, Workflow } from 'lucide-react';
...
const Automations = () => {
    ...
    const items = [
        {
            title: t.automations.cards[0].title,
            category: t.automations.cards[0].category,
            desc: t.automations.cards[0].desc,
            tags: ['n8n', 'GPT-4.1', 'Digisac', 'Bitrix24'],
            videoId: '14gpXqQutls',
            status: 'live',
        },
        {
        ...
        },
        {
            ...
            category: t.automations.cards[2].category,
            desc: t.automations.cards[2].desc,
            tags: ['n8n', 'API', 'CRM'],
            status: 'coming_soon',
        },
    ];
    ...
    return (
        <section className="py-12 lg:py-20 relative" id="automations">
            <div className="mx-auto max-w-7xl px-6">
                <div className="flex flex-col items-start text-left gap-4 mb-10 lg:mb-14">
                    <div className="flex items-center gap-4">
                        <Workflow className="size-4 text-primary" />
                        <h2 className="text-sm font-bold text-primary">{t.automations.badge}</h2>
                        <div className="h-px w-12 bg-primary/30" />
                    </div>
                    <h3 className="text-2xl sm:text-4xl lg:text-6xl font-bold text-white leading-tight">
                        {t.automations.title} <br />
                        <span className="gradient-text">{t.automations.titleAccent}</span>
                    </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    {items.map((item, index) => (
                        <VideoCard key={index} item={item} index={index} t={t} />
                    ))}
                </div>
            </div>
        </section>
